...
import net.tot3g.omclient.model.User;
import net.tot3g.omclient.service.MailEngine;
...
public class BaseFormController implements ServletContextAware {
    ...
    protected boolean allowpostpaidgetData(HttpServletRequest request)
    {
    	if(request.isUserInRole(Constants.ADMIN_ROLE)||request.isUserInRole(Constants.MANAGER_ROLE))    	
    		return true;    	
    	else
    		return "allow".equals(Constants.isAllowPostpaidSUBMIT);
    }
    
    protected boolean allowpostpaidsetData(HttpServletRequest request,SubscriberData sdata)
    {    	
    		return (!"allow".equals(Constants.isAllowPostpaidSUBMIT))&&!sdata.getMobileType().toLowerCase().equals("prepaid");
    }
	...
	protected static final class CorrelationIdPostProcessor implements MessagePostProcessor {
	...
	protected static final class ProducerConsumer implements SessionCallback<Message> {
